...
func (s *PostgresStore) GetLastRecordBySourceID(ctx context.Context, sourceID int64) (*model.DataRecord, error) {
	query := `
		SELECT id, source_id, token_id, data, ip_address, user_agent, created_at
		FROM data_records
		WHERE source_id = $1
...
func (s *PostgresStore) GetRecordsByIDRange(ctx context.Context, sourceID int64, startID, endID int64) ([]*model.DataRecord, error) {
	query := `
		SELECT id, source_id, token_id, data, ip_address, user_agent, created_at
		FROM data_records
		WHERE source_id = $1 AND id >= $2 AND id <= $3
		ORDER BY id ASC
	`
